...
=.h= files open in =c++-mode=. Indentation uses spaces throughout. ~company-c-headers~ provides header completion with paths auto-detected from the local compiler (=$CC=, GCC, or Clang) and qmake. Symbol navigation uses xcscope. =lsp-mode= also runs here, through =clangd=, alongside xcscope.
