...
This configuration depends on a number of external system packages.
 ...
 - [[https://www.gnu.org/software/global/][GNU Global]] for [[https://github.com/dkogan/xcscope.el][xcscope]] (via its =gtags-cscope= backend)
 ...
 - Mailcap for external viewer resolution
...
[[https://github.com/zeph1e/use-package-ensure-system-package-plus.el][use-package-ensure-system-package+]] runs these installs through one shell process, one at a time. Because of this, Emacs asks for your sudo password once per session, not once per package. =config/vterm.el= waits on this same queue: it does not compile the vterm module until its own system dependencies finish installing. A blocked sudo prompt during startup can look like a hang. It is not a hang.

[[https://github.com/purcell/exec-path-from-shell][exec-path-from-shell]] copies your shell's =$PATH= and other variables into Emacs, on macOS or X11 graphical sessions. It runs again after each install, through the same hook. As a result, a binary that =:ensure-system-package= just installed works right away, without a restart.

** System Environment

Use Emacs 30 or a later version. On Emacs 30 and later versions, Emacs loads ~use-package~ from its built-in copy. On older Emacs, =init.el= bootstraps [[https://github.com/radian-software/straight.el][straight.el]] and installs ~use-package~ through it.

** Compiler

The C/C++ header path detection needs GCC or Clang, found via =$CC= or on =$PATH=. With neither present, automatic include path discovery fails.
...
Almost all custom keybindings live in ~my:global-key-map~, a minor mode that always takes precedence over major and minor mode keymaps. Bindings go into ~:map my:global-key-map~ inside ~use-package~ ~:bind~ blocks — never ~global-set-key~ directly. The one exception is [[https://github.com/emacs-vs/block-travel][block-travel]]'s =M-p= / =M-n= bindings (see Custom Editing Commands below). They stay on the plain global map on purpose, so major and minor modes can still override them.
...
I customized the mode line myself, instead of using Powerline or similar packages. I wanted only the information I actually read:
 ...
 - Read-only indicator (lock icon, red background)
 - Buffer size
 - Modification flag
 - Buffer name
 - Line and column
 ...
 - Major mode
 - Process status (for example, a compilation buffer's running/exit state)
 - A [[https://github.com/TeMPOraL/nyan-mode][Nyan Cat]] progress bar, sized live to fill the remaining mode-line width as windows split or resize
 - Scroll percentage
 - The vc-mode branch
...
[[https://company-mode.github.io/][Company]] is enabled globally, with stock trigger settings. It appears on its own a moment after you type, and =C-;= also triggers it right away. You navigate candidates with =C-p= / =C-n=, and page through them with =C-v= / =M-v=. [[https://github.com/sebastiencs/company-box][company-box]] renders candidates with icons — a spell-check icon for ~company-ispell~ matches, and separate folder / file icons for ~company-files~ matches. [[https://github.com/company-mode/company-statistics][company-statistics]] reorders candidates by how often you picked them before.

A buffer in a mode with its own entry (see the mode-specific backends table below) tries that entry first. Next comes one merged entry: ~company-capf~, ~company-keywords~, ~company-dabbrev-code~, and ~company-gtags~ combine into a single candidate list, with ~company-yasnippet~ included. ~company-files~, ~company-ispell~, and ~company-dabbrev~ follow as three separate entries, tried one at a time until one of them returns candidates:

| ~company-backends~ entry                  | Backends                                                                                               |
|-------------------------------------------+--------------------------------------------------------------------------------------------------------|
| Mode-specific entry (if any, tried first) | See the mode-specific backends table below                                                             |
| Merged group for code context             | ~company-capf~, ~company-keywords~, ~company-dabbrev-code~, ~company-gtags~ (with ~company-yasnippet~) |
| Sequential fallbacks (tried in order)     | ~company-files~, ~company-ispell~, ~company-dabbrev~                                                   |

~company-gtags~ only returns candidates when ~cscope-minor-mode~ is on, because it reads the same GNU Global index as [[https://github.com/dkogan/xcscope.el][xcscope]].

The ~my:make-context-aware~ macro wraps each backend above in an :around advice, so it only fires in its matching context. The check has two parts: whether the buffer derives from ~prog-mode~ (or is listed in ~my:custom-prog-mode-list~), and whether point sits inside a string or comment.

| Backend set                                        | Code in prog-mode | String/comment in prog-mode | text-mode (Org, commit messages, ...) |
|----------------------------------------------------+-------------------+-----------------------------+---------------------------------------|
| capf, keywords, dabbrev-code, gtags                | Fires             | Skipped                     | Never fires                           |
| files, ispell, dabbrev                             | Skipped           | Fires                       | Always fires                          |

In practice, a plain ~text-mode~ buffer only ever gets ~company-ispell~, ~company-files~, and ~company-dabbrev~ candidates. The merged group never runs there. As a result, completion stays limited to spelling, file paths, and whole-word matches. Code buffers get the opposite split: the merged group runs on real code, and the fallback backends run only inside strings and comments.

For C/C++, ~company-c-headers~ uses compiler include paths detected at runtime. It runs the detected compiler — =$CC=, else =gcc=, else =clang= — with =-x <lang> -E -v -= and parses the output. It also finds Qt headers automatically when =qmake= is available. Mode-specific backends run ahead of the entries above, and are not subject to the context-aware wrapping:
...
When =M-.= or =M-,= adds a cursor on a match that is not visible, the view
moves to that match and centers it. After 1.5 seconds of idle time, or when
you give a different command, the view and the real cursor go back to the
first cursor. This also applies to the =-word= and =-symbol= variants.
...
[[https://www.emacswiki.org/emacs/RedoPlus][redo+]] (=plugins/redo+=) adds conventional linear redo on top of Emacs' built-in undo system. =C-_= runs undo, and =M-_= runs redo.
...
| =C-x ǀ= / =C-x _= | Split and move into new window ([[https://github.com/nex3/windsplit][windsplit]])   |
| =C-x x=           | Close current window                         |
| =C-{= / =C-}=     | Shrink / enlarge window horizontally         |

windcopy helps you duplicate a buffer into a split view without disturbing the other window. framemove hooks into windmove, so =C-<arrow>= wraps across frames automatically. A patch changes scrolling behavior. The first =C-v= or =M-v= that crosses the buffer boundary moves point to the end or the beginning, instead of an error. If you press =C-v= or =M-v= again after point reaches the boundary, Emacs signals the usual end-of-buffer or beginning-of-buffer error.

* Project Navigation & Search

** Projectile

[[https://github.com/bbatsov/projectile][Projectile]] provides project-aware file finding and buffer switching, with one-week caching. [[https://github.com/bbatsov/helm-projectile][helm-projectile]] provides the helm front end. Projectile discovers projects one directory level under =~/Workspace= (=projectile-project-search-path=). Both =C-x C-p= and =C-x p= open the command map.
...
[[https://github.com/dkogan/xcscope.el][xcscope]] provides C/C++ symbol cross-referencing through ~cscope-setup~. It runs its ~gtags-cscope~ backend, so indexing goes through GNU Global instead of the classic cscope database. It helps in large codebases where LSP indexing is impractical.
...
[[https://github.com/stevemolitor/claude-code.el][claude-code.el]] (=config/claude.el=) runs the [[https://github.com/anthropics/claude-code][Claude Code]] CLI inside Emacs. It binds the CLI under the =C-'= keymap prefix and opens it in a right-side window. [[https://github.com/stevemolitor/monet][monet]] bridges IDE-style features back into Emacs. Claude Code's process environment hooks into =monet-start-server-function=. =config/claude.el= has a =:custom= block, written as a comment, for =monet-diff-tool= (set to =monet-ediff-tool=) and =monet-diff-cleanup-tool= (set to =monet-ediff-cleanup-tool=). Monet runs with its own default diff tool instead. [[https://github.com/purcell/inheritenv][inheritenv]] propagates buffer-local environment variables, for example Tramp connections, into the Claude Code subprocess. Inside the Claude Code map, =M= cycles auto-accept, plan, and confirm modes (~claude-code-cycle-mode~).
...
[[https://emacs-lsp.github.io/lsp-mode/][lsp-mode]] is configured in =config/lsp.el= with the keymap prefix =C-c C-l=. Active language hooks: =c-mode=, =c++-mode=, =js-mode=, =typescript-mode=, =python-mode=, =web-mode=, =rust-mode=, =kotlin-mode=.
...
|-------------------+------------------------------+-------------------------------------------------------------------------------------------------------------|
| C / C++           | clangd                       | ~sudo apt install -y clangd~                                                                                |
...
[[https://emacs-lsp.github.io/lsp-ui/][lsp-ui]] provides hover documentation. Diagnostics render through [[https://www.flycheck.org/][Flycheck]] instead. =config/lsp.el= disables =lsp-ui-sideline-show-diagnostics=, and [[https://github.com/flycheck/flycheck-pos-tip][flycheck-pos-tip]] shows Flycheck's errors as a popup at point. [[https://github.com/emacs-lsp/helm-lsp][helm-lsp]] replaces ~xref-find-apropos~ with a helm workspace symbol search.
...
=.h= files open in =c++-mode=. Indentation uses spaces throughout, following Google's C/C++ style via the ~google-c-style~ package (=config/google.el=). ~company-c-headers~ provides header completion with paths auto-detected from the local compiler (=$CC=, GCC, or Clang) and qmake. Symbol navigation uses xcscope. =lsp-mode= also runs here, through =clangd=, alongside xcscope.
...
~my:rust-explain-error-at-point~ shows the =rustc --explain= output for the flycheck error at point, in a posframe popup. It syntax-highlights any embedded code blocks. When multiple errors overlap at point, it opens a picker posframe styled after company's tooltip faces instead (=C-n= / =C-p= to cycle, =C-g= to dismiss). The picker lists each error next to its explanation, and suppresses lsp-ui-doc's and flycheck's own popups while it is shown.
...
[[https://jblevins.org/projects/markdown-mode/][markdown-mode]] (=config/markdown.el=) beautifies =.md= buffers for reading. It hides markup characters near point. It reveals them again when point moves onto that line. As a result, raw asterisks and backticks do not show during normal reading. Heading faces use different colors and sizes per level, and switch palette automatically between light and dark themes. The concealment technique comes from [[https://www.reddit.com/r/emacs/comments/10h9jf0/beautify_markdown_on_emacs/][this r/emacs thread]].
...
=config/fileviewer.el= routes external file opens and URL navigation to the host's default viewer, with awareness of where Emacs is running:
 ...
 - *SSH-remote* (=$SSH_CLIENT= set) — URL navigation connects back to the workstation over SSH and runs =xdg-open= there (see SSH-remote setup below). Dired =V= instead runs the *local* =xdg-open=, and only when a graphical display is available. Otherwise, it skips the operation.
...
[[https://github.com/zeph1e/rfcview.el][rfcview]] (=config/rfcview.el=) formats IETF RFC text files for easier reading. Emacs installs it through use-package's =:vc= keyword, from the author's own fork rather than an ELPA archive.

* Translation

[[https://github.com/atykhonov/google-translate][google-translate]] (=config/google.el=, =M-+= / =C-+=) translates the region or the word at point. An advice guesses the source language from the text's Unicode range (Korean, Japanese, or German) and picks the matching target language from a configured set of direction pairs, so you don't have to pick languages by hand each time.
